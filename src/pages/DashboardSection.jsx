...
function DashboardSection({ transactions, categories, tokens }) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      {/* KPI stats — 2×2 on mobile, 4×1 on desktop */}
      ...
      {/* Chart area */}
      <ChartCard title="Flujo de Caja · 6 meses" height={chartHeight}>
        <ResponsiveContainer width="100%" height={chartHeight}>
          <AreaChart data={MONTHLY_DATA} margin={{ top: 4, right: 4, left: isMobile ? -20 : -8, bottom: 0 }}>
            <defs>
              <linearGradient id="gIncome"  x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%"  stopColor={tokens.chart.income}  stopOpacity={tokens.chart.areaOpacityHigh} />
                <stop offset="95%" stopColor={tokens.chart.income}  stopOpacity={0} />
              </linearGradient>
              <linearGradient id="gExpense" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%"  stopColor={tokens.chart.expense} stopOpacity={tokens.chart.areaOpacityHigh} />
                <stop offset="95%" stopColor={tokens.chart.expense} stopOpacity={0} />
              </linearGradient>
            </defs>
      ...
      {/* Recent transactions */}
      <GlassCard>
        <div style={{ fontSize: 11, color: tokens.text.tertiary, textTransform: "uppercase", fontWeight: 700, letterSpacing: "0.05em", marginBottom: 10 }}>
          Recientes
        </div>
        {recentTx.length === 0 ? (
